# this module ot be used to compare delta lists with each other
...
def concat_sheets(excel_file):
    ...
    df_one_file = pd.concat([file.parse(sheet_name) for sheet_name in visible_sheets])
    return df_one_file
...
def format_data(df):
    ...
    # df.drop(df.columns[0], axis=1, inplace=True)
    # ! these columns don't exist in the delta lists don't need to drop them
